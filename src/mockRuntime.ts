...
export class MockRuntime extends EventEmitter {
  ...
  public stack(startFrame: number, endFrame: number): IRuntimeStack {
    ...
    const instruction = line.includes("disassembly") ? this.instruction : undefined;
  ...
  private executeLine(ln: number, reverse: boolean): boolean {
    ...
    const reg0 = /\$([a-z][a-z0-9]*)(=(false|true|[0-9]+(\.[0-9]+)?|\".*\"|\{.*\}))?/gi;
    ...
    while ((matches0 = reg0.exec(line))) {
      if (matches0.length === 5) {
        ...
        if (access && accessType && accessType.includes(access)) {
